...
import { firstBidOfProduct, highestBidOfProduct } from './bid'
...
const addBidInfo = async (products) => {
  return await Promise.all(products.map(async product => {
    const firstBid = await firstBidOfProduct(product.id);
    if(firstBid){
      const highestBid = await highestBidOfProduct(product.id);
